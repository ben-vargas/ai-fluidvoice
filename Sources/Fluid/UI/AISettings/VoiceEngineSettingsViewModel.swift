...
final class VoiceEngineSettingsViewModel: ObservableObject {
    ...
    func saveGrokSTTAPIKey(_ key: String) throws {
        try GrokSTTKeychain.shared.storeAPIKey(key)
        self.settings.objectWillChange.send()
    }

    func deleteGrokSTTAPIKey() throws {
        try GrokSTTKeychain.shared.deleteAPIKey()
        self.settings.objectWillChange.send()
    }

    func hasSavedGrokSTTAPIKey() -> Bool {
        GrokSTTKeychain.shared.hasAPIKey
    }
}
