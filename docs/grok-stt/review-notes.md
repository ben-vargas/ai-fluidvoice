...
- **`.invalidAudio` swallow on the PCM chunk path is broader than "silent chunk"** (`Sources/Fluid/Services/GrokSTT/GrokSTTProvider.swift`). `transcribePCM` catches both `.emptyTranscript` and `.invalidAudio` and returns empty text (`GrokSTTProvider.swift:193`). `.emptyTranscript` is the true silent-chunk signal (200 + blank text). `.invalidAudio` is the generic HTTP-400 mapping, so a systematic request rejection (refused keyterm or `language`, malformed part) can leave `allTranscriptions` empty and `MeetingTranscriptionService` reports "Complete!" at 100% with no error. The same swallow reaches `transcribeSamplesForAPI` via the 1-arg `transcribeFinal`, so LocalAPI `/v1/transcribe` can answer HTTP 200 `{"text": ""}` for a request xAI rejected. Risk is limited: `GrokSTTKeytermBuilder` caps at 100 terms of <=50 chars, and `wireLanguageCode` filters auto/empty. Round 1 asked for this swallow; this is a narrowing suggestion, not a reversal. Suggested: keep the `.emptyTranscript` swallow and either drop `.invalidAudio` from the catch, or swallow it only after at least one meeting chunk has already succeeded.
